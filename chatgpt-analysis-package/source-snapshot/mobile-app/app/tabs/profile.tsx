...
export default function ProfileScreen() {
  ...
  const graphHistory = trackingHistory.data?.length ? trackingHistory.data : profileWeightGraphPoint(profile.data?.weight_kg);
...
function ProfileProgressPanel({
  ...
  const latestWeight = history.length ? metricValue(history[0], "weight_kg") || weight : weight;
  const oldestWeight = history.length > 1 ? metricValue(history[history.length - 1], "weight_kg") : null;
  const weightDelta = latestWeight && oldestWeight ? latestWeight - oldestWeight : 0;
  ...
  return (
    <AppCard style={{ gap: spacing.md, overflow: "hidden" }}>
      ...
      <View style={{ flexDirection: "row", gap: spacing.sm }}>
        <BodyMetricTile icon="scale" label="Weight" tone="green" value={latestWeight ? `${latestWeight.toFixed(1)} kg` : "Add weight"} />
      ...
      >
        <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: spacing.sm }}>
          ...
          <View style={{ borderRadius: radii.pill, backgroundColor: colors.primarySoft, paddingHorizontal: 12, paddingVertical: 7 }}>
            <Text style={{ color: colors.primary, fontSize: 12, fontWeight: "900" }}>
              {weightDelta === 0 ? "Baseline" : `${weightDelta > 0 ? "+" : ""}${weightDelta.toFixed(1)} kg`}
            </Text>
          </View>
        </View>
        ...
        <View style={{ flexDirection: "row", gap: spacing.md }}>
          <View style={{ flex: 1 }}>
            <MiniBarChart accent={colors.primary} history={history} metric="weight_kg" title="Weight trend" unit="kg" />
          </View>
...
function MiniBarChart({
  ...
  const values = points.map((item) => metricValue(item, metric));

...
from apps.accounts.models import Allergy, DietaryRestriction, DislikedFood, User, UserProfile
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with driver.session() as session:
            ...
            session.run("CREATE CONSTRAINT IF NOT EXISTS FOR (k:KnowledgeEntity) REQUIRE (k.type, k.key) IS UNIQUE")
            ...
            session.run("CREATE INDEX IF NOT EXISTS FOR (n:Nutrient) ON (n.normalized_key)")
            session.run("CREATE INDEX IF NOT EXISTS FOR (s:Supplement) ON (s.slug)")
            session.run("CREATE INDEX IF NOT EXISTS FOR (s:Supplement) ON (s.normalized_key)")
    ...
    def _sync_nutrients(self, session):
        ...
        nut_data = [
            {"id": n.id, "name": n.name, "slug": n.slug, "unit": n.unit, "normalized_key": normalize_token(n.slug)}
    ...
    def _sync_food_nutrients(self, session):
        ...
        fn_data = []
        rich_data = []
        for fn in FoodNutrient.objects.select_related("nutrient", "food").filter(food__is_active=True):
            amount = float(fn.amount)
            row = {"f_id": fn.food_id, "n_id": fn.nutrient_id, "amount": amount, "unit": fn.unit}
            fn_data.append(row)
    ...
    def _sync_supplements(self, session):
        ...
        sup_data = [
            {"id": s.id, "name": s.name, "slug": s.slug, "normalized_key": normalize_token(s.slug)}
    ...
    def _sync_supplement_nutrients(self, session):
        ...
        sn_data = [
            {"s_id": sn.supplement_id, "n_id": sn.nutrient_id, "amount": float(sn.amount), "unit": sn.unit}
            for sn in SupplementNutrient.objects.filter(amount__isnull=False, supplement__is_active=True, nutrient__is_active=True)
        ]
        if sn_data:
            session.run(
    ...
    def _sync_profile_links(self, session, user: User, profile: UserProfile):
        ...
        rest_links = [{"u_id": user.id, "d_id": d.id} for d in profile.dietary_restrictions.all()]
        if rest_links:
            session.run(
